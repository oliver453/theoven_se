...
export default function SearchBar({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedIndex >= 0) {
      // Om användaren har valt ett förslag med piltangenterna
  ...
  return (
    <div className="relative max-w-2xl mx-auto" ref={searchRef}>
      ...
      {showSuggestions && showSuggestionsList && (suggestions.articles.length > 0 || suggestions.categories.length > 0) && (
        <div className="absolute z-50 w-full mt-2 bg-background border border-foreground/20 rounded-lg shadow-lg max-h-96 overflow-y-auto">
          {/* Loading state */}
          {isLoading && (
